...
define(function(require) {
    var $ = require('jquery');
    var svg = require('svg');
    var faceData = require('data/faceData');
    var bgData = require('data/bgData');
    var bubbleData = require('data/bubbleData');
    var bodyData = require('data/bodyData');
    var clothData = require('data/clothData');
    var expressData = require('data/expressData');
    var eyebrowData = require('data/eyebrowData');
    var eyeData = require('data/eyeData');
    var featureData = require('data/featureData');
    var glassData = require('data/glassData');
    var hairData = require('data/hairData');
    var hatData = require('data/hatData');
    var hobbyData = require('data/hobbyData');
    var mouthData = require('data/mouthData');
    var noseData = require('data/noseData');
    ...
    var checkMenuItem = function(dom) {
        ...
        var $menuItem = $(dom);
        var index = parseInt($menuItem.data('menu'));
        if ($lastMenuItem) {
    ...
    var menuData = []; // 菜单项数据，下标从0开始分别表示发型、发色、脸型、肤色、眉毛、眼睛、、、

    var initMenu = function (data) {
        var dom = document.getElementById('menuFrame');
        if (!dom) {
            return false;
        }
        menuData = data || [];
        var html = [];
        for (var i = 0, d; d = menuData[i]; i++) {
	...
	var currentData;
	var refreshList = function (index) {
    ...
    var $lastItem; // 上一次表格项的选择项

    // 勾选用户选择的item，index 表格里面的第几项
	var checkListItem = function(index, list) {

        var $item = $('#' + 'listTd_' + index);
        if ($lastItem) {
	...
	var setSex = function (sex) {
        ...
        var dom = document.getElementById('menuFrame').children[0];
	    checkMenuItem(dom);
	};
